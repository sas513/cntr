import {
  users, categories, products, cartItems, orders, storeSettings, customerActivity, visitorStats,
...
export class MemStorage implements IStorage {
  ...
  private visitorStats: Map<number, VisitorStats> = new Map();
  ...
  async logActivity(insertActivity: InsertCustomerActivity): Promise<CustomerActivity> {
    ...
    const activity: CustomerActivity = { 
      ...insertActivity, 
      id,
      productId: insertActivity.productId || null,
      timestamp: new Date() 
    };
  ...
  async trackVisitor(visitor: InsertVisitorStats): Promise<VisitorStats> {
    const newVisitor: VisitorStats = {
      id: this.currentVisitorId++,
      ...visitor,
      timestamp: new Date(),
    };
    this.visitorStats.set(newVisitor.id, newVisitor);
    return newVisitor;
  }

  async updateVisitorStats(sessionId: string, pageViews: number): Promise<void> {
    const visitor = Array.from(this.visitorStats.values()).find(v => v.sessionId === sessionId);
    if (visitor) {
  ...
  async getVisitorStats(): Promise<{
    ...
    const visitors = Array.from(this.visitorStats.values());
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    const todayVisitors = visitors.filter(v => v.timestamp && v.timestamp >= today).length;
